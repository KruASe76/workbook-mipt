...
### Композиция
Пусть $f: B \rightarrow C$ и $g: A \rightarrow B$  
Тогда отображение $h: A \rightarrow C$ вида $h(x) := f(g(x)), x \in A$ называется **композицией** $f$ и $g$
...
Пусть $f: A \rightarrow B$ - инъекция, $\Gamma \subset A \times B$ - ее график  
Тогда отображение $f^{-1}: f(A) \rightarrow A$, графиком которого является $\tilde{\Gamma} = \set{(y, x) \enspace \vert \enspace (x, y) \in \Gamma}$ называется **обратным** $f$

### Полный прообраз
Пусть $f: A \rightarrow B$ (не обязательно инъекция)  
...
Если $\exists n \in \mathbb{N}$ и существует биекция $f: I_{n} \rightarrow A$, то $A$ называется **конечным** ($A = \varnothing$ - тоже конечное)  
Если множество $A$ не является конечным, то оно называется **бесконечным**

#### Утверждение
*$\mathbb{N}$ - бесконечное множество*

- Пусть $\exists n \in \mathbb{N}: \enspace \exists f: I_{n} \rightarrow \mathbb{N}$ - биекция
- Тогда $f^{-1}: \mathbb{N} \rightarrow I_{n}$ - тоже биекция
- Рассмотрим $g: I_{n+1} \rightarrow \mathbb{N}$ вида $g(k) = k$
    - $I_{n+1} \overset{g}{\rightarrow} \mathbb{N} \overset{f^{-1}}{\rightarrow} I_{n}$
- $g$ - инъекция
- Рассмотрим $h = f^{-1} \space \circ \space g$ - композиция инъекций $\Rightarrow$ инъекция
- По построению $h: I_{n+1} \rightarrow I_{n}$
- Тогда $n + 1 \leq n$
    - *Противоречие*

### Счетное множество
$A$ называется **счетным**, если существует биекция $f: \mathbb{N} \rightarrow A$ ($A$ равномощно $\mathbb{N}$)

$A$ называется **не более, чем счетным**, если существует инъекция $f: A \rightarrow \mathbb{N}$  
Любое не более чем счетное множество либо конечно, либо счетность

#### Теорема 2
*$\mathbb{N} \times \mathbb{N}$ счетно*

- Нужно построить биекцию $f: \mathbb{N} \times \mathbb{N} \rightarrow \mathbb{N}$
    ...
    2. биекцией
        ...
        - тогда $f: \mathbb{N} \times \mathbb{N} \rightarrow \mathbb{N}$ вида $f(m, n) = (2m - 1) \times 2^{n-1}$ - биекция
...
#### Утверждение 4
Если $A$ счетно и $f: A \rightarrow B$ - сюръекция, то $B$ не более чем счетно
...
- Построим инъекции $g: B \rightarrow A$ (существует биекция $h: A \rightarrow \mathbb{N}$)
    - $g(b) := \min h(E_{b})$
    - *wasted...*
...
- Докажем, что $\mathbb{Q}$ не более чем счетно
    ...
    - Докажем, что $\mathbb{Q}_{+}$ не более чем счетно
        - Рассмотрим $f: \mathbb{N} \times \mathbb{N} \rightarrow \mathbb{Q}_{+}, \enspace f(m, n) = \frac{m}{n}$
        - $f$ - сюръекция
